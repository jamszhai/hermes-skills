...
### Config Path Mismatch with Third-Party Tools

When a third-party tool (CC Switch, etc.) adds providers but Hermes can't
see them, the config paths don't match. Hermes Desktop App on Windows reads
from `C:\Users\<user>\AppData\Local\hermes\config.yaml` because
`HERMES_HOME` is set there; CC Switch writes to `~/.hermes/config.yaml`.

**CC Switch Hermes Sync Rule (user workflow):** If CC Switch is managing
Hermes providers, Hermes must be kept in sync from CC Switch. Treat CC
Switch as the source of truth for Hermes provider config; Hermes
`config.yaml` is derived from that source, not the other way around.

**CC Switch write bug (source-confirmed via `src-tauri/src/hermes_config.rs`):**
CC Switch does **section-level replacement** via `replace_yaml_section()`, not
blind append. Under normal conditions it reads the file, heals duplicate
top-level keys via `deduplicate_top_level_keys`, replaces an existing section
or appends a new one. Failure modes:

- `duplicate entry with key "custom_providers"` / `"model"` — two top-level sections
- `duplicate entry with key "base_url"` inside `custom_providers[0]` — a single
  list item contains duplicate keys because two provider records were merged
  during an interrupted write

**Do not** fix only the first form; nested duplicates require rebuilding the
whole list from a clean source.

Recommended remediation sequence:
1. Stop CC Switch; restore the Hermes-readable config to a clean, parseable state
2. Reset CC Switch's Hermes DB rows (providers / endpoints / health / logs for
   `app_type='hermes'`) so it skips live import on next start
3. If Hermes must reflect CC Switch providers, reconstruct `custom_providers`
   from CC Switch DB into the real config path, not from scratch
4. Do not hand-edit `~/.hermes/config.yaml` expecting CC Switch to respect it —
   it re-reads and overwrites on startup
...
**Third-party model managers writing to the wrong config path.** Tools
like **CC Switch** (a model-switching manager for Claude Code, Codex,
Gemini, etc.) write Hermes `custom_providers` to `~/.hermes/config.yaml`
but Hermes Desktop App on Windows reads from `C:\Users\<user>\AppData\Local\hermes\config.yaml`
(because `HERMES_HOME` is set to `AppData\Local\hermes`, not `~/.hermes`).
Result: providers added via CC Switch never appear in Hermes's model picker.

Fix pattern:
1. Confirm the real config path: `hermes config path`
2. Verify where the third-party tool writes: check its DB or log
3. Remove duplicate blocks: CC Switch appends a full second `custom_providers:` block to `~/.hermes/config.yaml` on each launch. `yaml.safe_load` silently returns the last value on duplicates — inspect raw lines first.
4. Re-populate CC Switch's SQLite so its startup sees `"<app> already has providers; live import skipped"` and stops writing.
5. (Optional) Write an independent file at `~/.hermes/config.yaml` so third-party writes don't touch the real config.
6. Set `HERMES_HOME` as a user environment variable for consistency.
