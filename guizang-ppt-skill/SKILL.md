...
#### ⚠️ 模板 JS 变量名冲突（拷贝后必须立即修复）

两个模板（`template.html` 和 `template-swiss.html`）的翻页导航 JS 中，**deck 容器变量名 `dk` 与 WebGL 的暗色背景绘制函数变量名 `dk` 冲突**：WebGL 部分声明了 `let dk=null`（暗色背景 draw 函数），而导航部分又声明了 `const dk=document.getElementById('deck')`（deck 容器元素）。JavaScript 引擎抛出 `Identifier 'dk' has already been declared` 错误，整个脚本块停止执行——**翻页、键盘事件、WebGL 背景全部失效，浏览器控制台无任何可见提示**。

**拷贝后必须立即修复**——将翻页导航中的 `const dk` 改为 `const deckEl`，并同步更新 `go()` 函数内所有 `dk.style.transform` 引用。示例如下：

```bash
# 在拷贝后的 index.html 中找到并修改两处：
# 1. 声明行（约第 530 行附近）
#    原: const dk=document.getElementById('deck'),sl=dk...
#    改: const deckEl=document.getElementById('deck'),sl=deckEl...
# 2. go() 函数内的 transform 引用
#    原: dk.style.transform=`translateX(${-ix*100}vw)`;
#    改: deckEl.style.transform=`translateX(${-ix*100}vw)`;
```

**不修复此冲突的后果**：页面加载后没有任何 JS 错误弹窗，但 ← → 翻页、ESC 索引、B 键低功耗全部不工作。症状是"页面好像卡在第一页，怎么操作都没反应"。

此项为 **Step 2 拷贝模板后的第一件事**，优先级高于修改 title 占位符。

#### ⚠️ Google Fonts CDN 在中国大陆无法访问

两个模板默认从 `fonts.googleapis.com` 加载衬线/非衬线字体（Noto Serif SC、Noto Sans SC、Playfair Display 等）。**该域名在中国大陆被 DNS 阻断**，导致字体无法加载，视觉效果与预期差异巨大。

**修复方案（二选一）**：

**方案 A（推荐：替换为 jsDelivr CDN）**：
```html
<style>
...
**方案 B（离线无依赖，纯系统字体）**：删除所有 `@import` 和 Google Fonts `<link>`，CSS 用以下回退：
```css
--serif-zh: "Source Han Serif SC","思源宋体",serif;
--sans-zh: "Source Han Sans SC","微软雅黑","Microsoft YaHei",sans-serif;
--mono: ui-monospace,monospace;
--serif-en: Georgia,serif;
```

**判断用户是否在中国**：如果用户使用中文提问、提及国内网络环境、或 Google/YouTube 等链接无法访问，必须选择方案 A 或 B。不要将 Google Fonts CDN 视为默认可用。

#### ⚠️ 模板 JS 变量名冲突（拷贝后必须立即修复）

两个模板（`template.html` 和 `template-swiss.html`）的翻页导航 JS 中，**deck 容器变量名 `dk` 与 WebGL 的暗色背景绘制函数变量名 `dk` 冲突**：WebGL 部分声明了 `let dk=null`（暗色背景 draw 函数），而导航部分又声明了 `const dk=document.getElementById('deck')`（deck 容器元素）。JavaScript 引擎抛出 `Identifier 'dk' has already been declared` 错误，整个脚本块停止执行——**翻页、键盘事件、WebGL 背景全部失效，浏览器控制台无任何可见提示**。

**拷贝后必须立即修复**——将翻页导航中的 `const dk` 改为 `const deckEl`（或其他不冲突的名字），并同步更新 `go()` 函数内所有 `dk.style.transform` 引用。示例如下：
...
两个模板默认从 `fonts.googleapis.com` 加载衬线/非衬线字体（Noto Serif SC、Noto Sans SC、Playfair Display 等）。**该域名在中国大陆被 DNS 阻断，导致字体无法加载**，浏览器 fallback 到系统衬线/非衬线字体（宋体、微软雅黑等），视觉效果与预期差异巨大（尤其是衬线大标题没有 Noto Serif SC 效果）。

**修复方案（二选一）**：

**方案 A（推荐）：替换为 jsDelivr CDN**——在 `<head>` 中将 Google Fonts 的 `@import` URL 替换为 jsDelivr 的 npm 包引用：
```html
<!-- 替换 fonts.googleapis.com 的 @import -->
<style>
  @import url('https://cdn.jsdelivr.net/npm/inter-ui@3.19.3/inter.min.css');
  /* 继续使用 fallback 字体栈：font-family fallback 到系统字体 */
  :root {
    --serif-zh: "Noto Serif SC","Source Han Serif SC","思源宋体",serif;
    --sans-zh: "Noto Sans SC","Source Han Sans SC","微软雅黑","Microsoft YaHei",sans-serif;
  }
...
**方案 B（离线无依赖）：完全移除 Google Fonts，纯用系统字体**——删除所有 `@import` 和 Google Fonts `<link>`，CSS 中字体栈用以下回退：
```css
--serif-zh: "Source Han Serif SC","思源宋体",serif;
--sans-zh: "Source Han Sans SC","微软雅黑","Microsoft YaHei",sans-serif;
--mono: ui-monospace,monospace;
...
6. **用 Lucide,不用 emoji**
7. **标题用衬线,正文用非衬线,元数据用等宽**\n8. **中文大标题字号必须双约束**——`font-size:min(Xvw, Yvh)`,只用 vw 在宽屏会溢出。推荐值：`h-hero` 用 `min(7vw,8vh)`（中文 2-3 行）、`h-xl` 用 `min(5vw,6.5vh)`、`h-sub` 用 `min(2.4vw,3.2vh)`。中文方块字视觉面积大，同样的 vw 值中文比英文看起来大 30-40%，必须酌情缩小。\n9. **正文字号用 `min(1.05vw,1.6vh)` 而非固定 vw**——确保 1080p 和 4K 屏幕上都可读
8. **中文大标题字号必须双约束**——`font-size:min(Xvw, Yvh)`,只用 vw 在宽屏会溢出。推荐值：`h-hero` 用 `min(7vw,8vh)`（中文 2-3 行）、`h-xl` 用 `min(5vw,6.5vh)`、`h-sub` 用 `min(2.4vw,3.2vh)`。中文方块字视觉面积大，同样的 vw 值中文比英文看起来大 30-40%，必须酌情缩小。
9. **正文字号用 `min(1.05vw,1.6vh)` 而非固定 vw**——确保 1080p 和 4K 屏幕上都可读
...
#### 呈现策略：内容即最终方案，不要标注"修订"

当用户要求修改已生成的内容（如删除某模块、改变某模块的实现方式）时，**修改后的网页 PPT 应直接呈现为最终方案，不要添加"修订说明""变更记录""修改说明"等过渡页或标注**。用户期望看到的是修改完成后的成品，而不是修改前后的对比过程。

**正确做法**：在 HTML 中直接替换/删除对应内容，确保所有页面呈现的是最终版本。
**错误做法**：新增一页标注"已删除""已修改"或"核心修订"，或在页面标题后加"(修订版)"后缀。

例外：只有当用户明确要求"展示修改前后对比"时，才使用并列对比布局呈现变更。
...
例外：只有当用户明确要求"展示修改前后对比"时，才使用并列对比（Layout 9）呈现变更。
